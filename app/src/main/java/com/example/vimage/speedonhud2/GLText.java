...
public class GLText {
    ...
    public boolean load(String file, int size, int padX, int padY, Boolean mirrorMode) {
        ...
        float y;
        // зеркальный режим текста
        if (mirrorMode) {
            y = +(cellHeight - 1) - fontDescent - fontPadY - cellHeight / 2;  // для зеркального режима
            canvas.scale(1f, -1f, 0, y);
        } else
            y = +(cellHeight - 1) - fontDescent - fontPadY;  // Set Start Position (Y)
        ...
        for (char c = CHAR_START; c <= CHAR_END; c++) {  // FOR Each Character
            ...
            if ((x + cellWidth - fontPadX) > textureSize) {  // IF End of Line Reached
                ...
                if (mirrorMode) {
                    ...
                    canvas.scale(1f, 1f, 0, y); // для зеркального отображения
                } else
                    y += cellHeight;                          // Move Down a Row
            }
        }
